...
class PolypeptideStructuralValidationTestCase(unittest.TestCase):
    ...
    def setUp(self):
        test_files = [
            str(TEST_FILE_FOLDER / x) for x in ["1ek9.pdb", "2ht0.pdb", "3qy1.pdb"]
        ]
        test_structures = [ampal.load_pdb(x) for x in test_files]
        self.test_polypeptides = [
            p
            for p in itertools.chain(*test_structures)
    ...
    def test_valid_distances_testing_files(self):
        ...
        valid_distances = [
            p.valid_backbone_bond_lengths() for p in self.test_polypeptides
        ]
